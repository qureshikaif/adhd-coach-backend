...
app.post('/api/quizzes', async (req: express.Request, res: express.Response) => {
  const { instructorId, title, questions } = req.body;

  try {
    const quizResult = await pool.query(
      'INSERT INTO quizzes (instructor, title) VALUES ($1, $2) RETURNING id',
      [instructorId, title]
    );
    ...
    const questionPromises = questions.map((q: any) =>
      pool.query(
        ...
        [quizId, q.question, q.option1, q.option2, q.option3, q.option4, q.answer]
      )
    );
...
app.get('/api/quizzes', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const quizzes = quizzesResult.rows.reduce((accumulator: any[], quiz: any) => {
      const existingQuiz = accumulator.find((item) => item.quiz_id === quiz.quiz_id);
      if (existingQuiz) {
